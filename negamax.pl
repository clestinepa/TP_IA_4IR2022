...
negamax(J,Etat,Pmax,Pmax,[_,Val]) :-		% cas 1
	heuristique(J,Etat,Val),
	!.

negamax(J,Etat,_,_,[_,Val]) :-				% cas 2
	situation_terminale(J,Etat),
	heuristique(J,Etat,Val),
	!.

negamax(J,Etat,_,_,[_,10000]) :-			% cas 2^(3/4) bis : pas besoin de jouer, J a gagné ! :)
	heuristique(J,Etat,10000),
	!.


negamax(J,Etat,_,_,[_,-10000]) :-			% cas 2^(3/4) bis : pas besoin de jouer, J a perdu... :(
	heuristique(J,Etat,-10000),									% 2^(3/4) en ref au quai 9^(3/4) : invisible mais indispensable ;)
	!.

negamax(J,Etat,P,Pmax,[Coup,V2]) :-		% cas 3
	P < Pmax,
	successeurs(J,Etat,Liste_successeur),
	loop_negamax(J,P,Pmax,Liste_successeur,Liste_coups),
	meilleur(Liste_coups,[Coup,V1]),
	V2 is -V1.
