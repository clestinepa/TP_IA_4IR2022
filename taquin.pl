...
%malplace(_, _, vide) :- false.
bienplace(U, F, P) :-
    coordonnees([Lig,Col], U, P),
    coordonnees([Lig,Col], F, P)
.
...
heuristique1(U, H) :-
    final_state(F),
    findall(P, (malplace(U, F, P), P \= vide), MalPlaces),
    length(MalPlaces, H)
.
...
manhattan(U, F, Elt, D) :-
    coordonnees([Lig_U, Col_U], U, Elt),
    coordonnees([Lig_F, Col_F], F, Elt),
    D is abs(Lig_F - Lig_U) + abs(Col_F - Col_U)
.
