...
heuristique(J,Situation,H) :-		% cas 3
	adversaire(J,Adv),
	findall(X,(alignement(X,Situation),possible(X,J),alignement_gagnant(X,J)),ListePossibleJ),
	findall(X,(alignement(X,Situation),possible(X,Adv),alignement_gagnant(X,Adv)),ListePossibleAdv),
	length(ListePossibleJ,HJ),
	length(ListePossibleAdv,HA),
	H is HJ - HA.