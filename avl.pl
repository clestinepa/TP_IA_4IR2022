...
empty(nil).
	...
	% elle est mise à jour de façon incrémentale, après chaque insertion ou suppression
...
put_90(avl(G,R,D,_H),Str) :-
	...
	%-----------------------------------------
	% Appartenance d'un element donné à un AVL	
	%-----------------------------------------
...
belongs(Elem, avl(G,Racine,D,_Hauteur)) :-
	...
	%------------------------------------------------------------
	% Recherche du sous-arbre qui a comme racine un élément donné	
	%------------------------------------------------------------
...
left_rotate(avl(G,R,D,_), A_Apres) :-
	...
	%---------------------------------
	% Insertion équilibrée dans un avl
	%---------------------------------
	...
	% L'insertion doit assurer qu'après insertion l'arbre est toujours équilibré
	% sinon les rotations nécessaires sont effectuées.

	% On suppose que les noeuds contiennent des informations que l'on peut comparer
	% à l'aide d'une relation d'ordre lexicographique (la clé c'est l'info elle-même)
